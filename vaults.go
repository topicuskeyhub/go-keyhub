...
func (s *VaultService) GetRecord(group *model.Group, uuid string, options model.VaultRecordAdditionalQueryParams) (result *model.VaultRecord, err error) {
	result, err = s.GetByUUID(group, uuid, &options)
	return
}

//  Retrieve a vault record by uuid for a certain group, including audit and secrets
func (s *VaultService) GetByUUID(group *model.Group, uuid string, additional *model.VaultRecordAdditionalQueryParams) (result *model.VaultRecord, err error) {
	results := new(model.VaultRecordList)
	errorReport := new(model.ErrorReport)

	url, _ := url.Parse(group.Self().Href)

	query := &model.VaultRecordQueryParams{UUID: uuid}
	if additional == nil {
		additional = &model.VaultRecordAdditionalQueryParams{Audit: true}
	}
...
func (s *VaultService) GetByID(group *model.Group, id int64, additional *model.VaultRecordAdditionalQueryParams) (result *model.VaultRecord, err error) {
	...
	if errorReport.Code > 0 {
		err = errors.New("Could not get VaultRecord '" + idString + "' of Group '" + group.UUID + "'. Error: " + errorReport.Message)
		return
	}
	if err == nil && al == nil {
		err = errors.New("VaultRecord '" + idString + "' of Group '" + group.UUID + "' not found!")
		return
	}
...
func (s *VaultService) Update(group *model.Group, vaultRecord *model.VaultRecord) (result *model.VaultRecord, err error) {
	...
	if errorReport.Code > 0 {
		err = errors.New("Could not update VaultRecord '" + vaultRecord.UUID + "' of Group '" + group.UUID + "'. Error: " + errorReport.Message)
		return
	}
